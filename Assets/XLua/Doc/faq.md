...
## lua代码文件保存什么格式
保存成 utf-8 格式，带bom格式的也可。
可以使用Noepad++直接转。

Windows平台上创建文本文件时，格式往往不是utf-8的。
这时候，在文件中编写 print("你好，世界") 这种代码就会出错，而关键是Unity控制台中什么都没有，这时候错误也没有提示。

初学者这种编码问题往往搞不好，大佬们又不会注意到这个细节，所以刚学习xlua的人遇到这种问题很被动。
